...
/*   Updated: 2024/02/01 19:09:34 by gsoteldo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void new_order_max_stack(t_stack *stacks, char flag)
{
	int index;
	...
	if(stacks->list_a->content != stacks->value->max_a)
	{
		if (stack_size(stacks->list_a) % 2 == 0)
		{
			if (index + 1 > stack_size(stacks->list_a) / 2)
...
void new_elem_in_a(t_stack *stacks, char flag)
{
	int index;
	
	stacks->moves->ra = 0;
	stacks->moves->rra = 0;
	index = find_index(stacks->list_a, search_number(stacks, stacks->list_b->content));
	if(stacks->list_a->content != search_number(stacks, stacks->list_b->content))
	{
		if (stack_size(stacks->list_a) % 2 == 0)
		{
			if (index + 1 > stack_size(stacks->list_a) / 2)
